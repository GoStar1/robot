...
namespace App\Libraries\Eip712;
...
class Encoder
{
    ...
    ): array
    {
        if (preg_match(Eip712::$ARRAY_REGEX, $type, $matches) === 1) {
            ...
            return ['bytes32', Keccak::hash(hex2bin(self::EncodeGroup($types, $values)), 256)];
        }
        ...
        if ($type === 'bytes') {
            return ['bytes32', Keccak::hash(hex2bin(substr($data, 2)), 256)];
        }
        return [$type, $data];
    }


    public static function encodeGroup($types, $values): string
    {
        $ethAbi = new Ethabi(['uint' => new Uinteger, 'bytes' => new Bytes(), 'address' => new Address, 'string' => new Str]);
        return substr($ethAbi->encodeParameters($types, $values), 2);
    }
    ...
    ): string
    {
        ...
        return self::EncodeGroup($types, $values);
    }
